function vr = dynamicSwitching_CA(vr)
...
if vr.numTrials - switches2use(end) > vr.minBlockSize % condition 1)
    ...
    if sum(vr.Rewards(window2check))/numel(window2check) >= vr.minFrac_Corr_4Switch  % condition 2)
        if vr.Rewards(vr.numTrials-1) == 1 % condition 3)
        % choose the other block from the current one
            ...
            vr.Switches = [vr.Switches vr.numTrials]; % append the switches vector
        end
    end
end
