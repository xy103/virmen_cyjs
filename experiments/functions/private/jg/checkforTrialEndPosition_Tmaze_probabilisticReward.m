function [vr] = checkforTrialEndPosition_Tmaze_probabilisticReward(vr)
...
if vr.inITI == 0 && (abs(vr.position(1))>4*vr.floorWidth)
    ...
    if vr.rewDelayTime > vr.rewardDelay   
        if mod(vr.currentWorld,2)==1 && vr.position(1)<0
            vr.isCorrect=1;
            % Give reward with probabiliy vr.correctRewardProbability
        ...
        elseif mod(vr.currentWorld,2)==0 && vr.position(1)>0
            ...
            vr.isCorrect=0;
            % Give reward with probabiliy vr.incorrectRewardProbability
            if rand <= vr.incorrectRewardProbability
                vr = giveReward(vr,1);
                vr.behaviorData(9,vr.trialIterations) = 1;
                vr.numRewards = vr.numRewards + 1;
            else
