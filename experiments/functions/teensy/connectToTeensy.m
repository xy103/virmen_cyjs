%% connectToTeensy: serial port connection to a Teensy
classdef connectToTeensy < handle
...
methods
	function obj = connectToTeensy(messgeCallbackFcn, baudRate, vr)
		obj.messgeCallbackFcn = messgeCallbackFcn;
		obj.arduinoMessageString = '';
		arduinoPortName = obj.findFirstArduinoPort(vr);
		...
		serialPort.BytesAvailableFcn = @(port, event)obj.readMessage(port, event, vr);
	...
	function readMessage(obj, port, event,vr)
...
% 		        if strfind(portFriendlyName, 'Teensy') %replaced arduino
% 		        for teensy and did not work
% 		            port = portName;
% 		            return
% 		        end
% 		    end
