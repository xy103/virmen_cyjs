...
vr.trialBlocki = 1;
vr.trialBlocks = [5, 5, inf];
vr.correctRewardProbabilities = [1.0 0.5 1.0];
vr.incorrectRewardProbabilites = [0.0 0.5 0.0];
vr.correctRewardProbability = vr.correctRewardProbabilities(vr.trialSequencei);
vr.incorrectRewardProbability = vr.incorrectRewardProbabilites(vr.trialSequencei);
...
vr = collectBehaviorIter_TMaze(vr, vr.correctRewardProbability);
...
if isTrialStart(vr)
    ...
    vr.correctRewardProbability = vr.correctRewardProbabilities(vr.trialBlocki);
    vr.incorrectRewardProbability = vr.incorrectRewardProbabilites(vr.trialBlocki);
end
