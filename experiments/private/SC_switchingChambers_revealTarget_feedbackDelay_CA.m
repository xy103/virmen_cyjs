...
%vr.mvThresh = eval(vr.exper.variables.mvThresh); % threshold for initiation of new trial
vr.friction = 0.25;
...
% try to introduce switches dynamically depending on performance of mouse
vr.minBlockSize = eval(vr.exper.variables.minBlockSize);

% General setup functions
%vr = initTextboxes(vr); % not working in new MATLAB version
vr = initDAQ(vr);
vr = initCounters(vr);
